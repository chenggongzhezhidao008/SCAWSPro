...
LogShow::LogShow(QWidget *parent) :
    QWidget(parent)
{
    _init_infor_show();
    _init_size();
...
void LogShow::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e)
    ...
    painter.drawText(m_x + m_width / 5,m_y,m_width * 14 / 20,m_height
                     ,Qt::AlignBottom | Qt::AlignLeft,"日志内容");
 ...
 * -----------------------------
 */
void LogShow::_m_log_information_update_page(int index)
{
    if(_m_showConfirm){
        return;
    }
...
void LogShow::_m_add_log_Slot(QString logCtx,QString optioner)
{
...
void LogShow::_m_add_logInformation(QString data)
{
    if(!data.isEmpty()){
        if(_m_log_number != 0){
...
void LogShow::_init_size()
{
    _m_width = 0;
    _m_height = 0;
}

void LogShow::_init_log_number()
{
    _m_log_number = 0;
}

void LogShow::_init_show_kind()
{
    _m_showConfirm = false;
}

void LogShow::_init_logIndex()
{
    _m_log_index = 0;
}
